...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	if (curl)
	{
		...
		{
			Auth_Error(va("Could not reach the RepZIW4.net server. Error code from CURL: %x.", code));
		}
...
void GetCountryCode()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://www.repziw4.de/getloc.php");
...
size_t CDataReceived(void *ptr, size_t size, size_t nmemb, void *data)
{
	...
	{
		char* message;
		sprintf(message, "This version (%d) has expired. Delete cachesSxS.xml and run RZOUpdaterV2.exe to obtain a new version (%d).\n", BUILDNUMBER, version);
		...
		if (result == IDYES)
		{
			...
			ShellExecute(NULL, "open", "RZOUpdaterV2.exe", 0, 0, SW_SHOWNORMAL);
			ExitProcess(1);
		}
		else if (result == IDNO)
		{
...
void DisableOldVersions()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://www.repziw4.net/bo-client/version.txt");
