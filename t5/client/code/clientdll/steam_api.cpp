...
extern "C"
{
	...
	__declspec(dllexport) bool __cdecl SteamAPI_Init()
	{
		...
		{
			Com_Error(1, "Could not connect to NP server at %s", MASTER_SERVER);
			return false;
		}
		...
		if (result->result != AuthenticateResultOK)
		{
			...
			case AuthenticateResultBadDetails:
				Com_Error(1, "Could not authenticate to NP server at %s -- bad details.", MASTER_SERVER);
				break;
			case AuthenticateResultAlreadyLoggedIn:
				Com_Error(1, "Could not authenticate to NP server at %s -- already logged in.", MASTER_SERVER);
				break;
			case AuthenticateResultBanned:
				Com_Error(1, "Could not authenticate to NP server at %s -- banned.", MASTER_SERVER);
				break;
			case AuthenticateResultUnknown:
				Com_Error(1, "Could not authenticate to NP server at %s -- unknown error.", MASTER_SERVER);
				break;
			}
		}
	...
	{
		Com_Printf(0, "RegisterCallResult for call ID %d", APICall);
	...
	{
		NP_SetRichPresence("current_server", "none");
		NP_SetRichPresence("isonline", "0");
